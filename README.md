...
Hover over a target and press **F8** to capture its desktop pixel coordinates; add each step, then save a named script. Semi translucent numbered crosshairs appear at each step's screen coordinates with the saved radius. With **Select crosshairs on screen** enabled, drag a marker to change that step's click position; its coordinates update in the editor immediately. Save the script to keep the moved position. Click a crosshair (or its row) to load that step into the editor, change its coordinates, radius, button or timings, and click **Update selected step**. Click **Save script** to keep the changes. Turn off **Select crosshairs on screen** to leave the markers visible while interacting with other apps; **Show numbered crosshairs** controls visibility. Markers pass clicks through during playback, or are hidden if the operating system cannot provide click-through. Desktop scripts loop back to step 1 by default until stopped. Clear **Loop until stopped** to use the saved finite repetition count. Set **Run timer seconds** to a positive value to stop after that much active playback time, or leave it at **0** for no time limit. **Start** shows a three-second countdown; a small always-on-top panel displays the countdown and elapsed timer with **Pause/Resume** and **Stop** buttons. **F10** pauses or resumes and **F9** stops even when the target app has focus. Pausing freezes the timer and releases a held mouse button; after resuming, that interrupted hold starts over. Set hold to **0 ms** and wait to **0 ms** for the fastest requested loop; actual click rate depends on the OS, hardware, and target app. The marker radius is a saved visual reference and does not change the click's physical footprint. The app uses standard synthetic input, which target apps may recognize. No detection bypass is implemented.
